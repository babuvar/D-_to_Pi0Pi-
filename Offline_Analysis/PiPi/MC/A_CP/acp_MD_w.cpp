...
//Common Pars
  RooRealVar m("#mu","m",1.86850);//,1.80,1.90);//
  RooRealVar s("#sigma","s",0.01535);//,0,0.04);//
  RooRealVar a("#alpha","a",0.769);//,0.5,0,2);
  RooRealVar n("n","n",10.1);//2,0,150);
...
//Full Model - N
  ...
  RooDataSet* combData =new RooDataSet("combData","combData",RooArgSet(dzero,w),Index(sample),Import("pos",*data_p),Import("neg",*data_n));
