...
//  RooFormulaVar s2("s2","0.01537*sigma_{fudge}",RooArgList(s_fudge));//**********
//  RooFormulaVar s2("s2","0.01579*sigma_{fudge}",RooArgList(s_fudge));//**********

  RooRealVar a("#alpha","a",0.743);//,0,2);
//  RooRealVar a("#alpha","a",0.720);//,0,2);//**********
//  RooRealVar a("#alpha","a",0.766);//,0,2);//**********

  RooRealVar n("n","n",12.8);//,0,150);
//  RooRealVar n("n","n",10.0);//,0,150);//**********
//  RooRealVar n("n","n",15.6);//,0,150);//**********

//Background
//Bin-1
  RooRealVar p("p","p",-0.5,-5,0); 
  RooRealVar mean_bkg("mean_bkg","mean_bkg",1.6799,1.0,1.9);
  RooRealVar sig_bkg("sig_bkg","sig_bkg",0.03381,0.02,0.2);
  RooRealVar f("frac_{Bkg1/Bkg2}","f",0.5,0,1); 
//Bin-2
  RooRealVar p2("p2","p2",-0.5,-5,0); 
  RooRealVar sig_bkg2("sig_bkg2","sig_bkg2",0.03381,0.02,0.2);
...
//Fit
  ...
  can->cd(1) ; gPad->SetLeftMargin(0.25) ; xframe_1->GetYaxis()->SetTitleOffset(1.4) ; xframe_1->Draw() ;
  can->cd(2) ; gPad->SetLeftMargin(0.25) ; xframe_2->GetYaxis()->SetTitleOffset(1.4) ; xframe_2->Draw() ;
  can2->cd(1) ; gPad->SetLeftMargin(0.25) ; xframe_3->GetYaxis()->SetTitleOffset(1.4) ; xframe_3->Draw() ;
  can2->cd(2) ; gPad->SetLeftMargin(0.25) ; TGaxis::SetMaxDigits(2); xframe_4->GetYaxis()->SetTitleOffset(1.4) ; xframe_4->Draw() ;

  TCanvas* can3 = new TCanvas("c3","c3",700,700) ;  can3->Divide(2,2) ;
  can3->cd(1) ; gPad->SetLeftMargin(0.15) ; xframe_1->Draw() ;
  can3->cd(2) ; gPad->SetLeftMargin(0.15) ; xframe_2->Draw() ;
  can3->cd(3) ; gPad->SetLeftMargin(0.15) ; xframe_3->Draw() ;
  can3->cd(4) ; gPad->SetLeftMargin(0.15) ; xframe_4->Draw() ;
