...
//Common
  ...
  RooRealVar Araw1("A_{Raw1}","Araw1",0,-1,1);
  RooRealVar N_t1("N_{Sig1}","N_t1",100,0,1000000);
  RooFormulaVar N_n("N_n","(0.5)*(1-A_{Raw1})*N_{Sig1}",RooArgList(Araw1,N_t1));
  RooFormulaVar N_p("N_p","(0.5)*(1+A_{Raw1})*N_{Sig1}",RooArgList(Araw1,N_t1));

  RooRealVar Araw2("A_{Raw2}","Araw2",0,-1,1);
  RooRealVar N_t2("N_{Sig2}","N_t2",100,0,1000000);
  RooFormulaVar N_n2("N_n2","(0.5)*(1-A_{Raw2})*N_{Sig2}",RooArgList(Araw2,N_t2));
  RooFormulaVar N_p2("N_p2","(0.5)*(1+A_{Raw2})*N_{Sig2}",RooArgList(Araw2,N_t2));
  ...
  RooFormulaVar N_nb2("N_nb2","(0.5)*(1-A_{Bkg1})*N_{Bkg2}",RooArgList(Abkg1,N_tb2));
  RooFormulaVar N_pb2("N_pb2","(0.5)*(1+A_{Bkg1})*N_{Bkg2}",RooArgList(Abkg1,N_tb2));
...
//Signal
  ...
  RooRealVar s_fudge2("sigma_{fudge2}","s_fudge2",1.0005,0.0,2.0);
  RooFormulaVar m("m","1.86861+mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s("s","0.01544*sigma_{fudge}",RooArgList(s_fudge));
  RooFormulaVar s2("s2","0.01570*sigma_{fudge}",RooArgList(s_fudge));
//  RooFormulaVar s2("s2","0.01544*sigma_{fudge2}",RooArgList(s_fudge2));
...
Float_t Asy, e_Asy, Asy2, e_Asy2;
Asy=Araw1.getVal();
e_Asy=Araw1.getError();
Asy2=Araw2.getVal();
e_Asy2=Araw2.getError();


cout<<"A_raw = "<<Asy<<" +/- "<<e_Asy<<endl;
cout<<"A_raw2 = "<<Asy2<<" +/- "<<e_Asy2<<endl;


double asym[10],easym[10], Den=0, Num=0, Avg, Sig;
...
for(int i=0;i<=1;i++){
Num=Num+(asym[i]/(easym[i]*easym[i]));
Den=Den+(1/(easym[i]*easym[i]));
}
Avg=Num/Den;
Sig=1/(sqrt(Den));
