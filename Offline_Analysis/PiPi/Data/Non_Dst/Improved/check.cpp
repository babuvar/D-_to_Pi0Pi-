...
//  RooRealVar dzero("dzero","dzero",1.68,2.06,"GeV");
  RooRealVar dzero("dzero","dzero",1.7,2.05,"GeV");
...
TH1D* data_p=new TH1D("pdat", "pdat", 100, 1.7, 2.05);
TH1D* data_n=new TH1D("ndat", "ndat", 100, 1.7, 2.05);
...
if(f_dzero >1.2 && f_dzero < 2.05){       //~3sigma range to estimate F.O.M.
...
//Common
  ...
  RooRealVar N_t("N_{Sig}","N_t",100000,0,10000000);
...
//Common Pars
  RooRealVar m_fudge("#mu_{fudge}","m_fudge",-0.001,-0.005,0.005);
  RooRealVar s_fudge("#sigma_{fudge}","s_fudge",1.4,1.1,1.7);


  RooFormulaVar m("m","1.86861+#mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s("s","0.01532*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a("#alpha","a",0.3,0.3,1.0);
  RooRealVar n("n","n",25,20,35);


  RooRealVar p("p","p",-0.20338,-5,0.02);
//  RooRealVar p("p","p",0.0);
  RooRealVar c("c","c",-20,-50,0.1);
  RooRealVar f("frac_{Bkg1/Bkg2}","f",0.9,0.7,1); 
