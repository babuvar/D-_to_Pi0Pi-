...
void acp_minos_data_forPlot2(void)
{
  //LOAD DATA FILE
...
num_pass++;
if(f_Dcharge == 1){data_p->add(dzero);}
if(f_Dcharge == -1){data_n->add(dzero);}
...
RooDataHist *data_P = data_p->binnedClone("data_P","data_P");
RooDataHist *data_N = data_n->binnedClone("data_P","data_P");
...
gr2->SetPointError(i, 0.0, 0.0, hres2->GetEYlow()[i] ,hres2->GetEYhigh()[i]);
    ...
    dframe3->SetMaximum(100) ;
    dframe3->SetMinimum(-100) ;
...
//    dframe3->SetMinimum(-1000) ;
    ...
    dframe4->SetMaximum(100) ;
    dframe4->SetMinimum(-100) ;
