...
double widthFF=1.069;//0.039
double al_sig=0.905;//0.013
double n_sig=2.469;//0.055 
double f_CBgaus=0.9619;//0.0022
double width_CBgaus=3.747;//0.099 
double mean_CBgaus=0.0313;//0.0035 
double al_bkg=-1.9750;//0.018
double n_bkgd=1.167;//0.020
...
void acp_minos_data_FFToy2(void)
{
  //LOAD DATA FILE
...
// al_sig=ran1->Gaus(0.905,0.013);
// n_sig=ran2->Gaus(2.469,0.055);
// f_CBgaus=ran3->Gaus(0.9619,0.0022);
// width_CBgaus=ran4->Gaus(3.747,0.099);
// mean_CBgaus=ran5->Gaus(0.0313,0.0035);
// al_bkg=ran6->Gaus(-1.9750,0.018);
// n_bkgd=ran7->Gaus(1.167,0.020);

 widthFF=(ran->Gaus(0.0,1.0) * 0.039) + 1.069;
 al_sig=(ran1->Gaus(0.0,1.0) * 0.013) + 0.905;
 n_sig=(ran2->Gaus(0.0,1.0) * 0.055) + 2.469;
 f_CBgaus=(ran3->Gaus(0.0,1.0) * 0.0022) + 0.9619;
 width_CBgaus=(ran4->Gaus(0.0,1.0) * 0.099) + 3.747;
 mean_CBgaus=(ran5->Gaus(0.0,1.0) * 0.0035) + 0.0313;
 al_bkg=(ran6->Gaus(0.0,1.0) * 0.018) - 1.9750;
 n_bkgd=(ran7->Gaus(0.0,1.0) * 0.020) + 1.167;
