...
float w;
//if(f_Df==1 || f_Df==10){w=0.695;}
if(f_Df==1 || f_Df==10){w=0.48;}
else{w=1.694;}
...
//Common
  ...
  RooRealVar Araw("A_{Raw}","Araw",-0.00,-1,1);
  RooRealVar N_t("N_{Sig}","N_t",71024,35000,145000);
  ...
  //____________________________________________________
  RooRealVar Abkg("A_{Bkg}","ABkg",-0.00,-1,1);
  RooRealVar N_tb("N_{Bkg}","N_tb",6390167,5500000,8000000);
...
cout<<"note: A_raw = "<<Asy<<" +/- "<<e_Asy<<endl;

  RooMCStudy* mcstudy = new RooMCStudy(simPdf,RooArgSet(dzero,sample),Binned(kTRUE),Silence(kTRUE),Extended(kTRUE), FitOptions(Save(kTRUE),PrintEvalErrors(0))) ;
...
//  RooPlot* frame2 = mcstudy->plotError(Araw,Bins(40)) ;
  RooPlot* frame2 = mcstudy->plotError(Araw,0.025,0.045,40) ;
...
//  RooPlot* frame5 = mcstudy->plotError(N_t,1700,1900,40) ;
  ...
  gStyle->SetPalette(1) ;
  gStyle->SetOptStat(0) ;
  TCanvas* can2 = new TCanvas("MC Study","MC Study",900,900) ;
  can2->Divide(2,2) ;
//  can->Divide(3,2) ;

  can2->cd(1) ; gPad->SetLeftMargin(0.15) ; frame1->GetYaxis()->SetTitleOffset(1.4) ; frame1->Draw() ;
  can2->cd(2) ; gPad->SetLeftMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1.4) ; frame2->Draw() ;
  can2->cd(3) ; gPad->SetLeftMargin(0.15) ; frame3->GetYaxis()->SetTitleOffset(1.4) ; frame3->Draw() ;
