...
//if(f_Egamma1 > 0.15 && f_Egamma2 > 0.15){
...
//Common
  ...
  RooRealVar N_t("N_{Sig}","N_t",70000,40000,130000);
...
//Common Pars
  ...
  RooRealVar s_fudge("#sigma_{fudge}","s_fudge",1.014);//,0.9,1.55);
  RooFormulaVar m("m_sig","1.87015+#mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s("s","0.01551*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a("#alpha","a",0.883);//,0.65,1.15);
  RooRealVar n("n","n",2.668);//,0,1000);
  RooFormulaVar m_g("m_g","1.87015+#mu_{fudge}+(0.02442*#sigma_{fudge})",RooArgList(m_fudge,s_fudge));
  RooFormulaVar s_g("s_g","0.0438*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar f_sig("f_sig","f_sig",0.9545);//,0.8,1.0);



  RooRealVar c("c","c",-4,-5,0.1);
//  RooRealVar m_b("#mu_bkg","m_b",1.6637,1.58,1.70);// P*(D) > 2.54
//  RooRealVar s_b("#sigma_bkg","s_b",0.04141,0.035,0.060);// P*(D) > 2.54
//  RooRealVar m_b("#mu_bkg","m_b",1.6637,1.55,1.72);// P*(D) > 2.65
//  RooRealVar s_b("#sigma_bkg","s_b",0.04141,0.028,0.063);// P*(D) > 2.65
  RooRealVar m_b("#mu_bkg","m_b",1.6637,1.55,1.72);// P*(D) > 2.7
  RooRealVar s_b("#sigma_bkg","s_b",0.04141,0.028,0.075);// P*(D) > 2.7



  RooRealVar a_b("#alpha_bkg","a_bkg",-1.9766);//,-2.0,-1.94);
  RooRealVar n_b("n_bkg","n_bkg",1.13);//,0.5,4.7);
  RooRealVar f_bkg2_p("f_bkg2_p","f_bkg2_p",0.93,0.91,1.0);
  RooRealVar f_bkg2_n("f_bkg2_n","f_bkg2_n",0.93,0.91,1.0);
