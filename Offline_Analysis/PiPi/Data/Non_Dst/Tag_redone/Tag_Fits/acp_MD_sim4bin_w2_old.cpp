...
//  RooRealVar dzero("dzero","dzero",1.68,2.00,"GeV");
  RooRealVar dzero("dzero","dzero",lowlim,2.06,"GeV");
...
void acp_MD_sim4bin_w2_old(void)
{
...
if(f_dzero > lowlim  && f_dzero < 2.06){       //~3sigma range to estimate F.O.M.
...
//if(f_Dcharge == 1){if(f_Df!=1){data_p->add(RooArgSet(dzero,w));}}
//if(f_Dcharge == -1){if(f_Df!=1){data_n->add(RooArgSet(dzero,w));}}
}
...
//if(f_Dcharge == 1){if(f_Df!=1){data_p2->add(RooArgSet(dzero,w));}}
//if(f_Dcharge == -1){if(f_Df!=1){data_n2->add(RooArgSet(dzero,w));}}
...
//Bin-1
  ...
  RooRealVar a("#alpha","a",0.911);//,0.65,1.15);
  RooRealVar n("n","n",4.39);//,0,1000);


//Bin-2
  RooFormulaVar m2("m_sig2","1.86790+#mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s2("s2","0.01740*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a2("#alpha2","a2",1.031);//,0.65,1.15);
  RooRealVar n2("n2","n2",2.70);//,0,1000);

//Background
//Bin-1
  RooRealVar p("p","p",-0.5,-5,0); 
  RooRealVar mean_bkg("mean_bkg","mean_bkg",1.6799,1.62,1.70);
  RooFormulaVar sig_bkg("sig_bkg","0.0362*#sigma_{fudgeBkg}",RooArgList(s_fudgeBkg));
//  RooRealVar sig_bkg("sig_bkg","sig_bkg",0.03381,0.02,0.06);
  RooRealVar f_p("fracP_{Bkg1/Bkg2}","f_p",0.5,0,1); 
  RooRealVar f_n("fracN_{Bkg1/Bkg2}","f_n",0.5,0,1); 
//Bin-2
  RooRealVar p2("p2","p2",-0.5,-5,0); 
  RooRealVar mean_bkg2("mean_bkg2","mean_bkg2",1.6799,1.64,1.70);
  RooFormulaVar sig_bkg2("sig_bkg2","0.043*#sigma_{fudgeBkg}",RooArgList(s_fudgeBkg));
//  RooRealVar sig_bkg2("sig_bkg2","sig_bkg2",0.03381,0.02,0.043);
  RooRealVar f2_p("frac2P_{Bkg1/Bkg2}","f2_p",0.5,0,1); 
  RooRealVar f2_n("frac2N_{Bkg1/Bkg2}","f2_n",0.5,0,1); 
...
//Background - P  
  ...
  RooAddPdf Bkg_p("Bkg_p","Bkg_p",RooArgList(bkg1_p,bkg2_p),RooArgList(f_p));
//Full Model - P
...
//Background - N  
  ...
  RooAddPdf Bkg_n("Bkg_n","Bkg_n",RooArgList(bkg1_n,bkg2_n),RooArgList(f_n));
//Full Model - N
  RooAddPdf Model_n("Model_n","Model_n",RooArgList(Sig_n,Bkg_n),RooArgList(N_n,N_nb));
//--------------------------------------------------------------------------------------
//Signal - P2
  RooCBShape Sig_p2("Sig_p2", "Cystal Ball Function", dzero, m2, s2, a2, n2); 
//Background - P2 
  RooChebychev bkg1_p2("bkg1_p2", "bkg1_p2", dzero, RooArgList(p2));
  RooGaussian bkg2_p2("bkg2_p2", " Gaussian 12",dzero,mean_bkg2,sig_bkg2);
//  RooGaussian bkg2_p2("bkg2_p2", " Gaussian 12",dzero,mean_bkg,sig_bkg);

  RooAddPdf Bkg_p2("Bkg_p2","Bkg_p2",RooArgList(bkg1_p2,bkg2_p2),RooArgList(f2_p));
//Full Model - P2
  RooAddPdf Model_p2("Model_p2","Model_p2",RooArgList(Sig_p2,Bkg_p2),RooArgList(N_p2,N_pb2));

//Signal - N2
  RooCBShape Sig_n2("Sig_n2", "Cystal Ball Function", dzero, m2, s2, a2, n2); 
//Background - N2
  RooChebychev bkg1_n2("bkg1_n2", "bkg1_n2", dzero, RooArgList(p2));
  RooGaussian bkg2_n2("bkg2_n2", " Gaussian 22",dzero,mean_bkg2,sig_bkg2);
//  RooGaussian bkg2_n2("bkg2_n2", " Gaussian 22",dzero,mean_bkg,sig_bkg);

  RooAddPdf Bkg_n2("Bkg_n2","Bkg_n2",RooArgList(bkg1_n2,bkg2_n2),RooArgList(f2_n));
//Full Model - N2
...
cout<<"Things are done-2, A_raw is "<<Araw.getVal()<<" +- "<<Araw.getError()<<endl;
  ...
  simPdf.plotOn(xframe_4,Slice(sample,"neg2"),Components("Bkg_n2"),ProjWData(sample,*combData_w),LineStyle(kDashed));
   Model_n2.paramOn(xframe_4,data_n2);
