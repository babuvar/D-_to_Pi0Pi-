...
void acp_MD_sim4bin_alt_v7(void)
{
  //LOAD DATA FILE
...
//Bin-1
  RooFormulaVar m("m_sig","1.86804+#mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s("s","0.01530*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a("#alpha","a",0.812);//,0.65,1.15);
  RooRealVar n("n","n",5.05);//,0,1000);
...
//  RooFormulaVar m_g("m_g","1.89+#mu_{fudge}",RooArgList(m_fudge,s_fudge));
  RooFormulaVar s_g("s_g","0.0652*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar f_sig("f_sig","f_sig",0.9735);//,0.8,1.0);


//Bin-2
  RooFormulaVar s2("s2","0.01494*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a2("#alpha2","a2",0.796);//,0.65,1.15);
  RooRealVar n2("n2","n2",3.80);//,0,1000);
...
//  RooFormulaVar m_g2("m_g2","1.86879+#mu_{fudge}",RooArgList(m_fudge,s_fudge));
  RooFormulaVar s_g2("s_g2","0.0642*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar f_sig2("f_sig2","f_sig2",0.9651);//,0.8,1.0);
