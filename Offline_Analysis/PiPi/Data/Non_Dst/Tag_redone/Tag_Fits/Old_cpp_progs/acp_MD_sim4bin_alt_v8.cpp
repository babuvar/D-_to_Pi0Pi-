...
void acp_MD_sim4bin_alt_v8(void)
{
  //LOAD DATA FILE
...
//if(f_Df==1 ||f_Df==10){w.setVal(0.0);}
else{w.setVal(1.45);}
//else{w.setVal(0.0);}
...
//  RooRealVar N_t1("N_{Sig1}","N_t1",0);
...
//  RooRealVar N_t2("N_{Sig2}","N_t2",0);
...
//  RooRealVar N_tb1("N_{Bkg1}","N_tb1",0);
...
//  RooRealVar N_tb2("N_{Bkg2}","N_tb2",0);
...
//Bin-1
  RooFormulaVar m("m_sig","1.86835+#mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s("s","0.01476*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a("#alpha","a",0.749);//,0.65,1.15);
  RooRealVar n("n","n",5.36);//,0,1000);
//  RooFormulaVar m_g("m_g","1.86805+#mu_{fudge}+(0.01885*#sigma_{fudge})",RooArgList(m_fudge,s_fudge));
//  RooFormulaVar m_g("m_g","1.89+#mu_{fudge}",RooArgList(m_fudge,s_fudge));
  RooFormulaVar s_g("s_g","0.0247*#sigma_{fudge}",RooArgList(s_fudge));
  RooFormulaVar s2_g("s2_g","0.0373*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar f_sig("f_sig","f_sig",0.9613);//,0.8,1.0);


//Bin-2
  RooFormulaVar m2("m_sig2","1.86920+#mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s2("s2","0.01421*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a2("#alpha2","a2",0.716);//,0.65,1.15);
  RooRealVar n2("n2","n2",4.04);//,0,1000);
//  RooFormulaVar m_g2("m_g2","1.86879+#mu_{fudge}+(0.02121*#sigma_{fudge})",RooArgList(m_fudge,s_fudge));
//  RooFormulaVar m_g2("m_g2","1.86879+#mu_{fudge}",RooArgList(m_fudge,s_fudge));
  RooFormulaVar s_g2("s_g2","0.01421*#sigma_{fudge}",RooArgList(s_fudge));
  RooFormulaVar s2_g2("s2_g2","0.0246*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar f_sig2("f_sig2","f_sig2",0.950);//,0.8,1.0);
...
//Bin-2
  RooRealVar p2("p2","p2",-0.4,-0.8,0.01);
...
//  RooGaussian Sig2_p("Sig2_p", "Sig2_p",dzero,m,s_g);
//  RooAddPdf Sig_p("Sig_p","Sig_p",RooArgList(Sig1_p,Sig2_p),RooArgList(f_sig));
  RooBifurGauss Sig2_p("Sig2_p", "Sig2_p",dzero,m,s_g,s2_g);
...
//  RooGaussian Sig2_n("Sig2_n", "Sig2_n",dzero,m,s_g);
//  RooAddPdf Sig_n("Sig_n","Sig_n",RooArgList(Sig1_n,Sig2_n),RooArgList(f_sig));
  RooBifurGauss Sig2_n("Sig2_n", "Sig2_n",dzero,m,s_g,s2_g);
...
//  RooGaussian Sig2_p2("Sig2_p2", "Sig2_p2",dzero,m,s_g2);
//  RooAddPdf Sig_p2("Sig_p2","Sig_p2",RooArgList(Sig1_p2,Sig2_p2),RooArgList(f_sig2));
  RooBifurGauss Sig2_p2("Sig2_p2", "Sig2_p2",dzero,m2,s_g2,s2_g2);
...
//  RooGaussian Sig2_n2("Sig2_n2", "Sig2_n2",dzero,m,s_g2);
//  RooAddPdf Sig_n2("Sig_n2","Sig_n2",RooArgList(Sig1_n2,Sig2_n2),RooArgList(f_sig2));
  RooBifurGauss Sig2_n2("Sig2_n2", "Sig2_n2",dzero,m2,s_g2,s2_g2);
