//root -l 'acp_MD_sim4bin_alt_v5_wCorr.cpp("out.txt")'
...
TVectorD X_sig(12), Y_sig(12), X_bkg(6), Y_bkg(6);
...
//---------------------------------------------------------------------
TMatrixD A_sig(12,12);
TMatrixD C_sig(12,12);
TMatrixD D_sig(12,12);
//---------------------------------------------------------------------
TMatrixD A_bkg(6,6);
TMatrixD C_bkg(6,6);
TMatrixD D_bkg(6,6);
...
void acp_MD_sim4bin_alt_v5_wCorr(char * filename)
{
  //LOAD DATA FILE
  TChain* chain=new TChain("h1");
//  TChain* chain=new TChain("h2");

   chain->Add("Root_Files_Data/pipi_dcont.root"); 
  chain->Add("Root_Files_Data/pipi_d4s.root"); 
  chain->Add("Root_Files_Data/pipi_d5s.root"); 
...
ifstream fin;
fin.open("Corr_tag_sig.txt");//dim=5;

while(!fin.eof())
{count++;i=count/12;j=count%12;
fin>>A_sig[i][j];
if(count==(12*12)-1)){break;}
}
fin.close();
//A_sig.Print();
...
//D_sig.Print();
//---------------------------------------------------------------------
fin.open("Corr_tag_bkg.txt");//dim=2;
count=-1;
while(!fin.eof())
{count++;i=count/6;j=count%6;
fin>>A_bkg[i][j];
if(count==(6*6)-1)){break;}
}
fin.close();
//A_bkg.Print();
...
X_sig[0]=ran->Gaus(0.0,1.0);
X_sig[1]=ran1->Gaus(0.0,1.0);
X_sig[2]=ran2->Gaus(0.0,1.0);
X_sig[3]=ran3->Gaus(0.0,1.0);
X_sig[4]=ran4->Gaus(0.0,1.0);
X_sig[5]=ran5->Gaus(0.0,1.0);
X_sig[6]=ran6->Gaus(0.0,1.0);
X_sig[7]=ran7->Gaus(0.0,1.0);
X_sig[8]=ran8->Gaus(0.0,1.0);
X_sig[9]=ran9->Gaus(0.0,1.0);
X_sig[10]=ran10->Gaus(0.0,1.0);
X_sig[11]=ran11->Gaus(0.0,1.0);

X_bkg[0]=ran12->Gaus(0.0,1.0);
X_bkg[1]=ran13->Gaus(0.0,1.0);
X_bkg[2]=ran14->Gaus(0.0,1.0);
X_bkg[3]=ran15->Gaus(0.0,1.0);
X_bkg[4]=ran16->Gaus(0.0,1.0);
X_bkg[5]=ran17->Gaus(0.0,1.0);
...
al1sig=  (Y_sig[0] * 0.014) + 0.805;
al2sig=  (Y_sig[1] * 0.026) + 0.781;
f1sig=   (Y_sig[2] * 0.003) + 0.9799;
f2sig=   (Y_sig[3] * 0.0051) + 0.9746;
mudif1=  (Y_sig[4] * 0.00028) + 0.00073;
mudif2=  (Y_sig[5] * 0.0086) + 0.0188;
mudif3=  (Y_sig[6] * 0.011) + 0.022;
n1sig=   (Y_sig[7] * 0.20) + 4.92;
n2sig=   (Y_sig[8] * 0.23) + 3.77;
sigrat1= (Y_sig[9] * 0.015) + 0.975;
sigrat2= (Y_sig[10] * 0.23) + 3.79;
sigrat3= (Y_sig[11] * 0.3) + 3.65;


al1bkg=    (Y_bkg[0] * 0.031) - 1.9014;
al2bkg=    (Y_bkg[1] * 0.038) - 1.9129; 
mudifbkg=  (Y_bkg[2] * 0.0022) - 0.00135;
n1bkg=     (Y_bkg[3] * 0.064) + 1.604;
n2bkg=     (Y_bkg[4] * 0.067) + 1.328;
sigratbkg= (Y_bkg[5] * 0.032) + 0.999;
