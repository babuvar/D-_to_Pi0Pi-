...
void fullfit_Imp35_data_b(void)
{
  //LOAD DATA FILE
...
//Common
  ...
  RooRealVar N_t("N_{Sig}","N_t",35000,25000,50000);
...
//Background 
  ...
  RooChebychev Bkg2("Bkg2", "Bkg2", dzero, RooArgList(p));
   RooRealVar m_b("#mu_bkg","m_b",1.6637,1.62,1.69);
  RooRealVar s_b("#sigma_bkg","s_b",0.04141,0.034,0.066);
  RooRealVar a_b("#alpha_bkg","a_bkg",-1.9766);//,-2.1,-1.8);
  RooRealVar n_b("n_bkg","n_bkg",1.13,0.9,2.1);
  RooCBShape Bkg3("Bkg3", "Bkg3", dzero, m_b, s_b, a_b, n_b); 

  RooRealVar f_bkg("f_bkg","f_bkg",0.93,0.0,1.0);
  RooRealVar f_bkg2("f_bkg2","f_bkg2",0.93,0.90,0.97);

//Full Background 
  RooAddPdf Bkg3p("Bkg3p","Bkg3p",RooArgList(Bkg1,Bkg2),RooArgList(f_bkg));
  RooAddPdf Bkg("Bkg","Bkg",RooArgList(Bkg3p,Bkg3),RooArgList(f_bkg2));
...
//Fit
 ...
 Model.plotOn(xframe_1,Components(Bkg3),LineStyle(kDashed),LineColor(kRed));
