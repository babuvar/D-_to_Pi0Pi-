...
//  RooAddPdf Bkg1("Bkg1","Bkg1",RooArgList(bkg1,bkg2),RooArgList(f_bkg));

//Peaking Background 
/*
   RooRealVar m_bkg_fudge("#mu_bkg_{fudge}","m_fudge",-0.0004119,-0.1,0.1);
  RooRealVar s_bkg_fudge("#sigma_bkg_{fudge}","s_fudge",1.072552,0.0,2.0);
  RooFormulaVar m_bkg("m_sig","1.6637+#mu_{fudge}",RooArgList(m_fudge));
  RooFormulaVar s_bkg("s_sig","0.04141*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar a_bkg("#alpha_{bkg}","a_bkg",-1.9766);//,0.65,1.15);
  RooRealVar n_bkg("n_{bkg}","n_bkg",1.13);//,0,1000);
*/


   RooRealVar m_b("#mu_bkg","m_b",1.6637);//,1.60,1.75);
  RooRealVar s_b("#sigma_bkg","s_b",0.04141,0.035,0.052);
  RooRealVar a_b("#alpha_bkg","a_bkg",-1.9766);//,-2.0,-1.94);
  RooRealVar n_b("n_bkg","n_bkg",1.13,0.7,2.7);
...
//Full Background 
  RooRealVar f_bkg2("f_bkg2","f_bkg2",0.93,0.91,1.0);
...
//   RooFitResult* fitRes = Sig.fitTo(*Data,Save());
//   RooFitResult* fitRes = Bkg.fitTo(*Data,Save());
...
//  Sig.plotOn(xframe_1);
//  Sig.paramOn(xframe_1);
//  Sig.plotOn(xframe_1,Components("Sig1"),LineStyle(kDashed));
//  Sig.plotOn(xframe_1,Components("Sig2"),LineStyle(kDashed),LineColor(kRed));

//  Model.plotOn(xframe_1);
//  Model.plotOn(xframe_1,Components("Sig"));
//  Model.paramOn(xframe_1);
//  Model.plotOn(xframe_1,Components("Sig1"),LineStyle(kDashed));
//  Model.plotOn(xframe_1,Components("Sig2"),LineStyle(kDashed),LineColor(kRed));

//  Bkg.plotOn(xframe_1);
//  Bkg.plotOn(xframe_1,Components(Bkg2),LineStyle(kDashed));
//  Bkg.plotOn(xframe_1,Components("bkg1"),LineStyle(kDashed));
//  Bkg.plotOn(xframe_1,Components("bkg2"),LineStyle(kDashed));
//  Bkg.paramOn(xframe_1);
...
// Model.plotOn(xframe_1,Components(bkg1),LineStyle(kDashed),LineColor(kOrange));
// Model.plotOn(xframe_1,Components(bkg2),LineStyle(kDashed),LineColor(kViolet));
   Model.paramOn(xframe_1);
