...
//  RooRealVar s_fudge("#sigma_{fudge}","s_fudge",1.072552,0.0,1.5);
//  RooFormulaVar m("m_sig","1.86805+#mu_{fudge}",RooArgList(m_fudge));
//  RooFormulaVar s("s_sig","0.015559*#sigma_{fudge}",RooArgList(s_fudge));

  RooRealVar m("#mu_{fudge}","m_fudge", 1.86775);
...
//  RooFormulaVar s_g("s_g","0.0441*#sigma_{fudge}",RooArgList(s_fudge));
  RooRealVar m_g("m_g","m_g",1.8915);//,0.65,1.15);
...
//Background 
  ...
  RooRealVar s_b("#sigma_bkg","s_b",0.04141,0.035,0.055);
  RooRealVar a_b("#alpha_bkg","a_bkg",-1.9766);//,-2.0,-1.94);
  RooRealVar n_b("n_bkg","n_bkg",1.13,0.5,2.7);
