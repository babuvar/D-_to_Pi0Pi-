...
using namespace RooFit; 
  ...
  RooRealVar PiMom("PiMom","PiMom",0.84,5.84,"GeV");
  RooDataSet* data_pipi=new RooDataSet("data_pipi","data_pipi",RooArgSet(PiMom,w));
  RooDataSet* data_kspi=new RooDataSet("data_kspi","data_kspi",RooArgSet(PiMom,w));
  RooDataSet* data_phipi=new RooDataSet("data_phipi","data_phipi",RooArgSet(PiMom,w));

void plots_mom(void)
{
int nsig_kspi=0, nsig_pipi=0, nsig_phipi=0;
...
Int_t nevt3=(int)chain->GetEntries();
 ...
 Float_t Deltam, f_PD, f_dzero, f_Pizsmass, f_Dstf, f_Egam1s, f_Egam2s, f_Egamma1, f_Egamma2, f_Categ, f_Pizmom, f_Pimom, f_Gam1thet, f_Gam2thet, f_Dcharge, f_Pizmass, f_Df, f_Ksmom, f_Phimom;
...
data_pipi->add(RooArgSet(PiMom,w));
...
data_kspi->add(RooArgSet(PiMom,w));
...
data_phipi->add(RooArgSet(PiMom,w));
...
//cout<<"---------------------------"<<endl;
 ...
 cout<<"nsig_phipi = "<<nsig_phipi<<endl;
  ...
  //DeltaM PLOTING
  RooPlot *xframe =PiMom.frame(Bins(50),Title("D^{+} #rightarrow #pi^{0} #pi^{+}"));
