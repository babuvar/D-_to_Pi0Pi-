...
void acp_MD(void)
{
  ...
  TFile *file = new TFile("KsPi_GMC0_4s.root");
//  TFile *file = new TFile("KsPi_GMC1_4s.root");
...
//Common Pars
  RooRealVar mean("mean","mean",1.86,1.8,1.9);
  RooRealVar sig("sig","sig",0.008,0.0,0.015);
  RooRealVar sig1("sig1","sig1",0.006,0,0.010);
  RooRealVar sig2("sig2","sig2",0.006,0,0.010);
  RooRealVar f_Sig("f_Sig","f_Sig",0.2012,0.0,1.0);



  RooRealVar p("p","p",0.0,-1.0,1.0); 
  RooRealVar c("c","c",-9,-30,10);
//  RooRealVar f("frac_{Bkg1/Bkg2}","f",0.0);
...
//  RooAddPdf Model_p("Model_p","Model_p",RooArgList(Sig_p,Bkg_p),RooArgList(N_p,N_pb));
  RooAddPdf Model_p("Model_p","Model_p",RooArgList(Sig_p,bkg1_p),RooArgList(N_p,N_pb));
//  RooAddPdf Model_p("Model_p","Model_p",RooArgList(Sig_p,bkg2_p),RooArgList(N_p,N_pb));
...
//  RooAddPdf Model_n("Model_n","Model_n",RooArgList(Sig_n,Bkg_n),RooArgList(N_n,N_nb));
  RooAddPdf Model_n("Model_n","Model_n",RooArgList(Sig_n,bkg1_n),RooArgList(N_n,N_nb));
//  RooAddPdf Model_n("Model_n","Model_n",RooArgList(Sig_n,bkg2_n),RooArgList(N_n,N_nb));
...
//Fit
  ...
  //DeltaM PLOTING
  RooPlot *xframe_1 =dzero.frame(Bins(30),Title("D^{+} #rightarrow K_{S} #pi^{+}"));
  ...
  RooPlot *xframe_2 = dzero.frame(Bins(30),Title("D^{-} #rightarrow K_{S} #pi^{-}"));
